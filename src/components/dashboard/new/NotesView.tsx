...
export default function NotesView({ textIntensity = 1.0, isDark = true, onCountChange }: NotesViewProps) {
  ...
  const btnBase = {
    ...
    border: `1px solid ${bg(0.12, isDark)}`, background: bg(0.04, isDark),
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <style>{`
        ...
        .mq-notes-scroll::-webkit-scrollbar-thumb { background: ${bg(0.10, isDark)}; border-radius: 2px; }
      `}</style>
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 16px 12px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button
                onClick={createNote}
                style={{ ...btnBase, width: 32, height: 32, borderRadius: 8, color: tw(0.5, textIntensity, isDark), fontSize: 18 }}
              >+</button>
              <button
                onClick={() => setIsListCollapsed(true)}
                style={{ ...btnBase, width: 32, height: 32, borderRadius: 8, color: tw(0.35, textIntensity, isDark), fontSize: 14 }}
              >{'\u00AB'}</button>
            </div>
          </div>
          ...
          <input
            ...
            style={{
              ...
              color: tw(0.7, textIntensity, isDark), fontSize: 12,
        ...
        <div className="mq-notes-scroll" style={{ flex: 1, overflowY: 'auto', padding: '0 8px 12px' }}>
          {filtered.length === 0 && isLoaded && (
            <div style={{ textAlign: 'center', padding: '24px 8px', color: tw(0.25, textIntensity, isDark), fontSize: 12 }}>
          ...
          {filtered.map(note => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = bg(0.04, isDark) }}
                onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = 'transparent' }}
              >
                <div style={{
                  fontSize: 14, fontWeight: 500, color: tw(0.85, textIntensity, isDark),
                  whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                }}>
                  {note.title || 'Sans titre'}
                </div>
                <div style={{
                  fontSize: 12, color: tw(0.35, textIntensity, isDark), marginTop: 2,
                  whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                }}>
                  {stripHtml(note.content).slice(0, 60) || 'Note vide'}
                </div>
                <div style={{ fontSize: 11, color: tw(0.2, textIntensity, isDark), marginTop: 3 }}>
        ...
        {selectedNote ? (
          <>
            ...
            <div style={{
              ...
              {isListCollapsed && (
                <>
                  <button
                    ...
                    style={{ ...btnBase, width: 34, height: 34, borderRadius: 6, color: tw(0.4, textIntensity, isDark), fontSize: 14 }}
                  >{'\u00BB'}</button>
              ...
              <button
                ...
                style={{ ...btnBase, width: 34, height: 34, borderRadius: 6, color: tw(0.5, textIntensity, isDark), fontSize: 14, fontWeight: 700 }}
              >G</button>
              {/* Italic */}
              <button
                onClick={() => execCmd('italic')}
                style={{ ...btnBase, width: 34, height: 34, borderRadius: 6, color: tw(0.5, textIntensity, isDark), fontSize: 14, fontStyle: 'italic' }}
              >I</button>
              {/* Underline */}
              <button
                onClick={() => execCmd('underline')}
                style={{ ...btnBase, width: 34, height: 34, borderRadius: 6, color: tw(0.5, textIntensity, isDark), fontSize: 14, textDecoration: 'underline' }}
              >S</button>
              ...
              <button
                onClick={highlight}
                style={{ ...btnBase, width: 34, height: 34, borderRadius: 6, color: tw(0.5, textIntensity, isDark), fontSize: 13 }}
                title="Surligner"
              >
              ...
              <button
                ...
                style={{ ...btnBase, width: 34, height: 34, borderRadius: 6, color: tw(0.5, textIntensity, isDark), fontSize: 14, textDecoration: 'line-through' }}
              >B</button>
              ...
              <button
                onClick={deleteNote}
                style={{
                  ...btnBase, width: 34, height: 34, borderRadius: 6,
                  color: tw(0.25, textIntensity, isDark), border: 'none', background: 'transparent',
                }}
                onMouseEnter={e => {
            ...
            <div className="mq-notes-scroll" style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
              ...
              <div style={{ fontSize: 12, color: tw(0.25, textIntensity, isDark), marginBottom: 20, display: 'flex', gap: 8 }}>
                <span>{formatDate(selectedNote.updated_at, 'full')}</span>
                {savedIndicator && (
              ...
              <div
                ...
                style={{
                  minHeight: 300, fontSize: 15, lineHeight: '1.75',
          ...
          <div style={{
            ...
            {isListCollapsed && (
              <button
                onClick={() => setIsListCollapsed(false)}
                style={{ ...btnBase, width: 34, height: 34, borderRadius: 6, color: tw(0.4, textIntensity, isDark), fontSize: 14, marginBottom: 8 }}
              >{'\u00BB'}</button>
            )}
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke={tw(0.15, textIntensity, isDark)} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
