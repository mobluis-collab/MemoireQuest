...
import { calculateLevel, getLevelProgress } from '@/lib/xp/levels'
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? '0 8px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.07)' : 'none',
        cursor: 'pointer',
      }}>
      {/* Liquid fill */}
      <div style={{
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', fontWeight: 700, width: 16, flexShrink: 0 }}>
          {ch.num}
        </span>
        <span style={{
          flex: 1, fontSize: 13,
              ...
              return (
                <div key={i}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {isNext && !isDone && (
                      <div style={{ fontSize: 10, color: 'rgba(99,102,241,0.8)', marginTop: 2, fontWeight: 600 }}>
                        Cliquer pour valider ↗
                      </div>
                    )}
                    {isDone && (
                      <div style={{ fontSize: 10, color: 'rgba(52,211,153,0.6)', marginTop: 2 }}>Terminé</div>
                    )}
                  </div>
                  {/* Difficulty badge */}
                  <span style={{
                    fontSize: 9, fontWeight: 700, flexShrink: 0, padding: '2px 6px', borderRadius: 99,
                    background: sec.difficulty === 'hard'
                      ? 'rgba(251,113,133,0.15)'
                      : sec.difficulty === 'medium'
                        ? 'rgba(251,191,36,0.15)'
                        : 'rgba(52,211,153,0.15)',
                    color: sec.difficulty === 'hard' ? C.rose
                      : sec.difficulty === 'medium' ? C.amber : C.emerald,
                  }}>
...
export default function NewDashboard({
  ...
  const nextLevelXP = levelInfo.isMaxLevel ? totalPoints : (totalPoints + (levelInfo.xpRequiredForNext - levelInfo.xpInCurrentLevel))
  ...
  return (
    <div style={{
      fontFamily: FONT, height: '100vh', overflow: 'hidden',
      position: 'relative', display: 'flex', background: '#04030e',
    }}>
      {/* ── CSS animations ── */}
      <style>{`
        @keyframes mq-drift {
      ...
      <aside style={{
        ...
        <div style={{ padding: '24px 18px 16px', borderBottom: '1px solid rgba(255,255,255,0.055)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '-0.3px', color: 'rgba(255,255,255,0.9)' }}>
                MemoireQuest
              </div>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.3px' }}>
                Thesis OS v1.0
              </div>
            </div>
          </div>
        ...
        <div style={{ padding: '14px 14px 12px', borderBottom: '1px solid rgba(255,255,255,0.055)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{firstName}</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.32)', marginTop: 1 }}>
                Niv. {currentLevel} · {currentLevel <= 2 ? 'Étudiant' : currentLevel <= 4 ? 'Chercheur Jr.' : currentLevel <= 7 ? 'Chercheur' : 'Expert'}
              </div>
            </div>
          </div>
          {/* XP bar */}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)' }}>{totalPoints} XP</span>
            <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)' }}>
              {levelInfo.isMaxLevel ? 'Max !' : `${nextLevelXP} XP · niv. ${currentLevel + 1}`}
            </span>
          </div>
        </div>
        ...
        <div style={{ padding: '8px 7px', borderTop: '1px solid rgba(255,255,255,0.055)' }}>
          <button
            ...
            style={{
              width: '100%', display: 'flex', alignItems: 'center', gap: 9,
              padding: '8px 11px', borderRadius: 9, border: 'none', cursor: 'pointer',
              background: 'transparent', color: 'rgba(251,113,133,0.65)', fontSize: 12, textAlign: 'left',
            }}>
      ...
      <main style={{
        ...
        {plan ? (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              ...
              <div style={{
                ...
                <span style={{ fontSize: 15, fontWeight: 800, color: C.amber, letterSpacing: '-0.5px' }}>
                  {streak.current}
                </span>
            ...
            <div style={{
              ...
              >
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 10 }}>
                      Soutenance dans
                    </div>
                    ...
                    <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.2px' }}>
                      Deadline · <span style={{ color: 'rgba(255,255,255,0.5)', fontWeight: 500 }}>{fmt(deadlineDate, 'long')} {deadlineDate.getFullYear()}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)' }}>{fmt(startDate)}</span>
                      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', fontWeight: 600 }}>{timePct}% du temps écoulé</span>
                      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)' }}>{fmt(deadlineDate)}</span>
                    </div>
                  </div>
                </div>
              </GBorder>
              ...
              <GBorder gradient="linear-gradient(135deg, rgba(56,189,248,0.4), rgba(99,102,241,0.2), rgba(4,3,14,0.1))">
                <div style={{
                  ...
                  <Arc pct={pct} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.28)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 16 }}>
                      Avancement
                    </div>
                    {[
                      ...
                      <div key={s.label} style={{
                        ...
                        marginBottom: 8, gap: 8, whiteSpace: 'nowrap',
                      }}>
              ...
              >
                <div style={{
                  ...
                  {[
                    { label: 'Niveau',   val: String(currentLevel), sub: currentLevel <= 2 ? 'Étudiant' : currentLevel <= 4 ? 'Chercheur Junior' : currentLevel <= 7 ? 'Chercheur Confirmé' : 'Expert ès Mémoires', col: C.indigo },
                    { label: 'XP Total', val: String(totalPoints),  sub: levelInfo.isMaxLevel ? 'Niveau maximum !' : `${levelInfo.xpRequiredForNext - levelInfo.xpInCurrentLevel} XP avant niv. ${currentLevel + 1}`, col: C.violet },
                    { label: 'Série',    val: `${streak.current}j`, sub: `Jokers disponibles : ${streak.jokers}`, col: C.amber },
                  ].map((s, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.28)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>
                        {s.label}
                      </div>
                      <div style={{
                        fontSize: 36, fontWeight: 900, letterSpacing: '-1px', lineHeight: 1,
                        color: '#fff', textShadow: `0 0 28px ${s.col}77`, marginBottom: 5,
                      }}>{s.val}</div>
              ...
              <div style={{ gridColumn: '1/3' as unknown as undefined, display: 'flex', flexDirection: 'column', gap: 7, minHeight: 0 }}>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', flexShrink: 0 }}>
                  Chapitres
                </div>
          ...
          /* No plan: Upload zone centered in dark shell */
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {isLoading ? (
              <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>Génération du plan en cours…</div>
            ) : (
