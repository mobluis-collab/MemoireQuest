...
export async function PUT(request: Request) {
  ...
  if (!hasAccentColor && !hasTextIntensity) {
    return NextResponse.json({ error: "At least one field required: accent_color or text_intensity" }, { status: 400 });
  }
  ...
  const { error } = await supabase.from("user_preferences").upsert(updatePayload);
